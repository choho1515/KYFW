const DBHandler = function () {
    ...
    DBHandler.prototype.init = function () {
        const [DBF, SM, LDB, _chat] = this.args
        const chat = _chat.data
        let tar = []
        let now = (this.time / 1000)
        if (now - chat.created_at > 3600) {
            return
        } else if (now - chat.created_at > 10) {
            this.active = false
        } else this.active = true
        const _room = DBF.get('chat_rooms', chat.chat_id)
        const room = _room.data
        /*if (room.type == 'DirectChat') {
        }*/

        if (room.type != 'OM') return this.exit('not openchat')
        try {
            room.name = JSON.parse(room.private_meta).name
        } catch (e) {}
        if (!room.name) return this.exit('custom name not set')
        if (room.name[0] != '●' && room.name[0] != '■') return this.exit('room name separator not exist');

        //decrypt chat
        _chat.decrypt()

        if (chat.v.isMine) return this.exit('own msg')

        //if (room.name != '■캬옹봇 디버깅') return

        //개선사항: 실제로 답장할만한 메세지인 경우만 복호화
        const _user = DBF.get('friends', chat.user_id).decrypt()
        ...
        new java.lang.Thread({
            run: function () {
                try {
                    execute([DBF, SM, LDB, _chat, _user, _room, chat, user, room, _U, _R, _RU, U, R, RU])
